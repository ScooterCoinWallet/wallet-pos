...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " scootercoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  scootercoin-tx [options] <hex-tx> [commands]  Update hex-encoded scootercoin transaction\n" +
            "or:     scootercoin-tx [options] -create [commands]   Create hex-encoded scootercoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded scootercoin transaction
